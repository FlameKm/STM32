...
void button_changed(uint32_t button_state, uint32_t has_changed)
{
    ...
    // 检查是否有左、上、右、下按钮按下
    // 如果有任何按钮按下，将鼠标位置结构体的值相应地修改，并标记有数据要发送
    if (buttons & KEY_LEFT_MASK) {
...
void main(void)
{
    ...
    /* 设备信息服务(DIS)在系统启动时使用SYS_INIT宏初始化。 */
    hid_init();
    ...
    /*     for (int i = 0; i < ARRAY_SIZE(btn); i++) {
        if (!device_is_ready(&btn[i])) {
            printk("Failed to btn %d ready", i);
        }
        err = gpio_pin_configure_dt(&btn[i], GPIO_INPUT);
        if (err < 0) {
            printk("Failed to btn %d config", i);
        }
        err = gpio_pin_get_dt(&btn[i]);
        if (err < 0) {
            printk("Failed to btn %d read pin", i);
        }
        gpio_init_callback(&callback, press_button, BIT(btn[i].pin));
        err = gpio_add_callback(&btn[i], &callback);
        if (err < 0) {
            printk("Faild to add callback");
        }
        err = gpio_pin_interrupt_configure_dt(&btn[i], GPIO_INT_EDGE_TO_ACTIVE);
        if (err < 0) {
